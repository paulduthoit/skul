...
module.exports.hideFields = function(fields, data, params) {
	...
	var displayedKeys = ['name', 'email', 'vehicles','last_vehicle_bound','order', 'organisation'];

	// TODO: Faire la recursive pour les models en dessous etc..

	console.log('test')
	console.log(hasManyRelationshipsKeys)
	console.log(relationshipsKeys)
		...
		_.each(dataItem, function(dataValue, dataKey) {
			...
			if (
				...
				if (!_.contains(displayedKeys, dataKey)) delete dataItem[dataKey];
					...
					console.log('HAS MANY RL', dataKey)
					if (typeof dataItem[dataKey] === 'object') {
						console.log(self.getRelationship(dataKey))
					}
