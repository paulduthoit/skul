...
module.exports.addRelationship = function(type, key, relationship) {
	...
	// If type is hasOneCustom
	else if(type === 'hasOneCustom') {
		...
		if(!(relationship.joinRaw instanceof Array))
	...
	// If type is hasManyCustom
	else if(type === 'hasManyCustom') {
...
module.exports.getRelationshipKeys = function(type) {
	...
	if (!(
			...
			|| (typeof type === "string" && _.contains([ 'belongsTo', 'hasMany', 'hasOne', 'hasManyCustom', 'hasOneCustom', 'hasManyThrough', 'hasOneThrough' ], type))
			|| (type instanceof Array && _.intersection([ 'belongsTo', 'hasMany', 'hasOne', 'hasManyCustom', 'hasOneCustom', 'hasManyThrough', 'hasOneThrough' ], type).length == type.length)
		)) {
			throw new Error("type have to be a string or an array defined as / including belongsTo, hasMany, hasOne, hasManyCustom, hasOneCustom, hasManyThrough or hasOneThrough");
		}

	// Return typped relationship keys
	if (type) {
		return _.compact(_.map(this.relationships, function(obj, key) { 
			if (typeof type == 'string') return obj.type === type && key;
			else if (type instanceof Array) return _.contains(type, obj.type) && key;
			else return null;
		}));
	}
