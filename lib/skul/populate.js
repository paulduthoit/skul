...
var _populateOneToManyCustomRelationship = function(key, data, filter, fields, options, params) {
	...
	var relJoinType = relJoin.type;
	var relJoinTable = relJoin.table;
	...
	var getRelationshipIdsLoad = function() {
		...
		return new Promise(function(resolve, reject) {
			...
			// Set join raw
			privateMethods._addCustomJoin(subQueryBuilder2 ,relModel, relJoin, relOn, improvedHashAlias, selfAlias);
	...
	var checkArgumentsLoad = function() {
		...
		// Add to filter
		newFilter = {};
		newFilter[relPrimaryKey] = { '$in': _.uniq(_.flatten(_.values(relHashId))) };
	...
	var countHashLoad = function() {
		...
		return new Promise(function(resolve, reject) {
			...
			// Set join raw
			privateMethods._addCustomJoin(subQueryBuilder ,relModel, relJoin, relOn, improvedHashAlias, selfAlias);
