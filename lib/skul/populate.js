...
var _populateOneToManyRelationship = function(key, data, filter, fields, options, params) {
	...
	var countHashLoad = function() {
	    ...
	    return new Promise(function(resolve, reject) {
    		...
    		queryBuilder.addColumn('B.' + relForeignKey + ' AS A$' + relForeignKey);
			...
			// Query
			dbConnection.query(queryBuilder, function(err, queryResult) {
	...
	var listHashLoad = function() {
	    ...
	    return new Promise(function(resolve, reject) {
		    ...
		    var subQueryBuilder = null;
		    var subQueryBuilder2 = null;

		    // Create sub query builder
	    	subQueryBuilder = relModel.createSelectQuery(filter, fields, options, hashAlias);

	    	// Get sub query builder 2
	    	subQueryBuilder2 = subQueryBuilder.getFromRaw().queryBuilder;

	    	// Add relationship foreign key if needed
	    	if(!_.contains(hashFields, relForeignKey)) {
	    		subQueryBuilder2.addColumn('A.' + relForeignKey + ' AS A$' + relForeignKey);
	    	}

	    	// Add variable increments
	    	subQueryBuilder2.addColumn(knex.raw('@num := IF(@foreign = `A`.`' + relForeignKey + '`, @num + 1, 0) AS row_number'));
	    	subQueryBuilder2.addColumn(knex.raw('@foreign := `A`.`' + relForeignKey + '` AS dummy'));

	    	// Add orderby
			subQueryBuilder2.addOrderBy('`A`.`' + relForeignKey + '` ASC');

		    // Remove all offset and limit statement
		    subQueryBuilder.setLimit(null);
		    subQueryBuilder.setOffset(null);

		    // Move last query statement
		    // var lastQueryStatement = subQueryBuilder._statements.pop();   /////////// CAN BE A BUG /////////
		    // subQueryBuilder._statements.unshift(lastQueryStatement);

		    // Create main query
		    mainQueryBuilder = mainQueryBuilder.select();
		    mainQueryBuilder = mainQueryBuilder.from(knex.raw(subQueryBuilder.toString()).wrap('(', ') AS C'));
			mainQueryBuilder = mainQueryBuilder.whereRaw('row_number >= ' + options.skip + ' AND row_number < ' + (options.skip + options.limit));
...
var _populateOneToManyCustomRelationship = function(key, data, filter, fields, options, params) {
	...
	var getRelationshipIdsLoad = function() {
		...
		return new Promise(function(resolve, reject) {
    		...
    		subQueryBuilder2.setHavingRaw(privateMethods._parseFilter.call(relModel, subQueryBuilder2, filter, hashAlias, 'A$1'));
	...
	var countHashLoad = function() {
		...
		return new Promise(function(resolve, reject) {
    		...
    		subQueryBuilder.setHavingRaw(privateMethods._parseFilter.call(relModel, subQueryBuilder, filter, hashAlias, 'A$1'));
