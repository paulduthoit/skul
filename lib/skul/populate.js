...
var _populateOneToManyRelationship = function(key, data, filter, fields, options, params) {
	...
	var countHashLoad = function() {
	    ...
	    return new Promise(function(resolve, reject) {
    		...
    		queryBuilder.addColumn('C.' + relForeignKey + ' AS A$' + relForeignKey);
	...
	var listHashLoad = function() {
	    ...
	    return new Promise(function(resolve, reject) {
	    	...
	    	if(!_.contains(hashFields, relForeignKey)) {
	    		subQuery2Builder.getFromRaw().queryBuilder.addColumn('A.' + relForeignKey + ' AS A$' + relForeignKey);
	    	}
	    	...
	    	subQueryBuilder.setFromRaw('(' + subQuery2Builder.toString() + ') AS `C`');

	    	// Add variable increments
	    	subQueryBuilder.addColumn(knex.raw('@num := IF(@foreign = `C`.`A$' + relForeignKey + '`, @num + 1, 0) AS row_number'));
	    	subQueryBuilder.addColumn(knex.raw('@foreign := `C`.`A$' + relForeignKey + '` AS dummy'));

	    	// Add orderby
			subQueryBuilder.addOrderBy('`C`.`A$' + relForeignKey + '` ASC');

		    // Create main query
		    mainQueryBuilder.addColumn(knex.raw('*'));
		    mainQueryBuilder.setFromRaw('(' + subQueryBuilder.toString() + ') AS `D`');
	    	...
	    	if(subQuery2Builder.getOrderBy().length === 0) {
	    		mainQueryBuilder.setOrderBy([ '`D`.`A$' + relPrimaryKey + '`' ]);
	    	} else {
