	...
	_.each(fields, function(obj, key) {
		...
		asyncQueue = asyncQueue
			.then(function() {
				...
				// Populate belongsTo/hasOne/hasOneCustom relationship
				if(_.contains(belongsToRelationshipKeys, key) || _.contains(hasOneRelationshipKeys, key) || _.contains(hasOneCustomRelationshipKeys, key)) {
	...
	var countHashLoad = function() {
	    return new Promise(function(resolve, reject) {
		    ...
		    /* FROM */
			    ...
			    // Set alias
			    var tableName = context.mainModel.getTableName();
			    var tablePrimaryKey = context.mainModel.getPrimaryKey();
			    var tableAlias = 'T' + context.aliasIncrement;
				...
				/* NEW */
					...
					// Set data
					var subSubQueryBuilder = (function() {

						// Set data
						var subqueryBuilder = new QueryBuilder();
						var subqueryBuilderColumns = {};

					    // Add query builder columns
					    subqueryBuilderColumns[tablePrimaryKey] = 'X$primary_key';
					    subqueryBuilderColumns[relForeignKey['$key']] = 'X$rel_key';

						// Set columns
						subqueryBuilder.setColumns(_.map(subqueryBuilderColumns, function(value, key) { return key + ' AS ' + value; }));

					    // Set from
						subqueryBuilder.setFromRaw('`' + tableName + '`');
				    ...
				    // Set from
					context.queryBuilder.setFromRaw(subSubQueryBuilder);
					context.queryBuilder.addJoin('raw', 'LEFT JOIN `' + tableName + '` AS `' + tableAlias + '` ON `' + subSubQueryBuilder.getAlias() + '`.`X$primary_key` = `' + tableAlias + '`.`' + tablePrimaryKey + '`');

				    // Add query builder columns
				    context.queryBuilderColumns[subSubQueryBuilder.getAlias() + '.' + 'X$rel_key'] = tableAlias + '$' + '_rel_key';
		    ...
		    /* CUSTOM JOIN */
				...
				var customAlias = tableAlias + '$' + '_rel_key';
				var subqueryBuilder = new QueryBuilder();
				...
				subqueryBuildingTasks.push(function() {
	    			...
	    			// Add rel id
	    			/*if(relForeignKey['$alias'] === relAlias) {
	    				subqueryBuilder.addColumn(knex.raw('`' + context.queryBuilder.getAlias() + '`.`' + 'X$rel_key' + '` AS `' + customAlias + '`'));
	    			} else {
				...
				subqueryBuildingTasks.push(function() {
					...
					// Add rel key
					subqueryBuilder.addGroupByRaw('`' + context.queryBuilder.getAlias() + '`.`' + context.mainAlias + '$_rel_key`');

					// Add rel id
					/*if(relForeignKey['$alias'] === relAlias) {
						subqueryBuilder.addGroupByRaw('`' + context.queryBuilder.getAlias() + '`.`' + context.mainAlias + '$' + relForeignKey['$key'] + '`');
					} else {
    		...
    		console.log('queryBuilder:count', context.queryBuilder);
	    ...
	    return new Promise(function(resolve, reject) {
		    ...
		    /* CUSTOM JOIN */
				...
				subqueryBuildingTasks.push(function() {
	    			...
	    			// Add rel id
	    			if(relForeignKey['$alias'] === relAlias) {
	    				subqueryBuilder.addColumn(knex.raw('`' + context.queryBuilder.getAlias() + '`.`' + context.mainAlias + '$' + relForeignKey['$key'] + '` AS `' + customAlias + '`'));
	    			} else {
	    				subqueryBuilder.addColumn(knex.raw('`' + relForeignKey['$alias'] + '`.`' + relForeignKey['$key'] + '` AS `' + customAlias + '`'));
	    			}
		    ...
		    /* WHERE */
				...
				whereClause += '`' + tableAlias + '$' + '_rel_key' + '` IN (' + (hashId.length > 0 ? hashId.join(',') : '-1') + ')';
		    ...
		    /* ORDERBY */
		    	...
		    	// Add rel foreign key order by
				context.queryBuilder.setOrderBy([ '`' + tableAlias + '$' + '_rel_key' + '` ASC' ]);
		    ...
		    /* RELATIONSHIP */
				...
				var queryBuilderAlias = context.queryBuilder.getAlias();
				var mainAliasItem = _.find(context.hashAlias, function(obj) { return obj.key === ''; });
				var relForeignKeyAlias = mainAliasItem.alias + '$' + '_rel_key';
    		...
    		console.log('queryBuilder:list', context.queryBuilder);
			...
			return dbConnection.query(queryBuilder, function(err, queryResult) {
				...
				_.each(queryResult, function(queryItem) {
					...
					// Push to object
					hashData[queryItem['_rel_key']].data.push(queryItem);

					// Remove _rel_key
					delete queryItem['_rel_key'];
...
var _populateOneToOneRelationship = function(key, data, filter, fields, options, params) {
	...
	var relationship = self.relationships[key];
	var relModel = objectPath.get(relationship, [ 'model', '$model' ]) || objectPath.get(relationship, [ 'model' ]);
	var relTableName = relModel.getTableName();
	var relForeignKey = relationship.foreignKey;
	var dataToPopulate = data instanceof Array ? data : [ data ];
