...
var _populateOneToManyRelationship = function(key, data, filter, fields, options, params) {
    ...
    if(typeof fields === "undefined" || fields === null) fields = { '$all': 1 };
	    ...
	    return new Promise(function(resolve, reject) {
		    ...
		    /* ORDER BY */
		    	...
		    	if(orderbyItems) {
					_.each(orderbyItems, function(orderbyItem) {
						...
						// Set order by string
						orderbyString.push('`' + absoluteHashAliasItem.alias + '$' + fieldName + '`' + (orderbyIsAsc ? ' ASC' : ' DESC'));
		    ...
		    /* ORDERBY */
				...
				if(orderbyString.length > 0) {
					_.each(orderbyString, function(val) {
						context.queryBuilder.addOrderBy(val);
					});
				}
