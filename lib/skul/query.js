...
module.exports.createSelectQuery = function(filter, fields, options, hashAlias) {
	...
	if(parsedOptions.limit !== null) {
		subQueryBuilder.setLimit(parsedOptions.limit);	
	}

	// Add offset clause
	if(parsedOptions.skip !== null){
		subQueryBuilder.setOffset(parsedOptions.skip);
	}
	...
	if(parsedOptions.orderby !== null) {
		...
		_.each(parsedOptions.orderby, function(key) {
			...
			var orderByItem = _.find(hashAlias, function(obj) { return obj.key === fieldBase; }).alias + '$' + fieldName + ' ' + (isAsc ? 'DESC' : 'ASC');
	...
	////////////// CAUTION : GROUP BY DOES NOT WORK /////////////////
