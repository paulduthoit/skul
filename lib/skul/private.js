    ...
    _.each(fields, function(fieldValue, fieldKey) {
    	...
    	// If field is one2many relationship and is grouped
			...
			self.queryBuildingTasks[1].push(function() {
    			...
    			var subjoinOnFieldPath = subJoinOnField.split('.');
				...
				subqueryBuilder.addJoin('raw', [
					...
					'ON `' + self.queryBuilder.getAlias() + '`.`' + parentAlias + '$' + model.getPrimaryKey() + '` = `' + joinAlias + '`.`' + subjoinOnAliasItem.alias + '$' + fieldRelationship.foreignKey + '`'
				]);
