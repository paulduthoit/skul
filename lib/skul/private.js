...
var _parseFields = function(model, fields, parentAlias, parentField) {
	...
	// var defaultKeys = model.getStructureColumns();
    	...
    	// If field is one2many relationship and is grouped
			...
			self.queryBuildingTasks[1].push(function() {
    			...
    			if(objectPath.get(fieldValue, '$count')) {
    				subqueryBuilder.addColumn(knex.raw('COUNT(`' + joinAlias + '`.`' + fieldRelationship.foreignKey + '`) AS `' + calculatedColumn + '`'));
    			} else if(objectPath.get(fieldValue, '$sum')) {
    				subqueryBuilder.addColumn(knex.raw('SUM(`' + joinAlias + '`.`' + fieldValue['$sum'] + '`) AS `' + calculatedColumn + '`'));
    			} else if(objectPath.get(fieldValue, '$avg')) {
    				subqueryBuilder.addColumn(knex.raw('AVG(`' + joinAlias + '`.`' + fieldValue['$avg'] + '`) AS `' + calculatedColumn + '`'));
    			}
	...
	// If $key is defined
		...
		_.each(filterKeys, function(filterKey, filterKeyIndex) {
			...
			if(typeof objectPath.get(filterValue, '$where') !== 'undefined') {
				...
				// Add where
				whereClause += _parseFilterItem.call(self, model, filterValue['$as'], filterValue['$where'], parentAlias, parentField);
 ...
 * @private
 */
var _parseFilterItem = function(model, itemKey, itemValue, parentAlias, parentField) {

    // Set data
    var self = this;

	// Parse filter item
	var parsedKey = _parseFilterKey.call(self, model, itemKey, parentAlias, parentField);
	...
	if(!(new RegExp('CAST\\(.*? AS DATE\\)').exec(parsedKey)) && new RegExp('CAST\\(.*? AS DATE\\)').exec(parsedValue)) {
		return 'CAST(' + parsedKey + ' AS DATE) ' + parsedFilter + ' ';
 ...
 * @private
 */
var _parseFilterKey = function(model, itemKey, parentAlias, parentField) {
	...
	// If absolute hash alias item if undefined
