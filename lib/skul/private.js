...
var _parseFilterValue = function(model, itemValue, parentAlias, parentField) {
	...
	// If LIKE
		...
		if(itemValue['$like'].match(/^\%|\%$/)) whereClause += ' LIKE ' + knex.raw('?', [ itemValue['$like'] ]) + ' ';
		else whereClause += ' LIKE ' + knex.raw('?', [ '%' + itemValue['$like'] + '%' ]) + ' ';
	}

	// If NOT LIKE
	else if(_.contains(_.keys(itemValue), '$nlike')) {
		if(itemValue['$nlike'].match(/^\%|\%$/)) whereClause += ' NOT LIKE ' + knex.raw('?', [ itemValue['$nlike'] ]) + ' ';
		else whereClause += ' NOT LIKE ' + knex.raw('?', [ '%' + itemValue['$nlike'] + '%' ]) + ' ';
	}
