...
var _parseFields = function(model, fields, parentAlias, parentField) {
    ...
    _.each(fields, function(fieldValue, fieldKey) {
			    ...
			    if(fieldRelationship.type === 'belongsTo') {
					self.queryBuildingTasks[0].push(function() {
						self.queryBuilder.addJoin('left', [
							...
							joinAlias + '.' + fieldRelationship.model.getPrimaryKey(),
					...
					self.queryBuildingTasks[0].push(function() {
						self.queryBuilder.addJoin('left', [
							fieldRelationship.model.getTableName() + ' AS ' + joinAlias,
							parentAlias + '.' + model.getPrimaryKey(),
    	...
    	// If field is one2many relationship and is grouped
			...
			self.queryBuildingTasks[1].push(function() {
    			...
    			if(objectPath.get(fieldValue, '$count')) {
    				var subJoinRequiredField = fieldRelationship.foreignKey;
    			} else if(objectPath.get(fieldValue, '$sum')) {
    				var subJoinRequiredField = fieldValue['$sum'];
    			} else if(objectPath.get(fieldValue, '$avg')) {
    				var subJoinRequiredField = fieldValue['$avg'];
    			}
    			subjoinFields[subJoinRequiredField] = 1;

    			// Set subjoin builder
    			var subjoinBuilder = fieldRelationship.model.createSelectQuery(subjoinFilter, subjoinFields, { limit: null, skip: null }, [], subjoinContext);

    			// Get subjoin on alias
    			var subjoinOnFieldPath = subJoinRequiredField.split('.');
    			var subjoinOnFieldName = subjoinOnFieldPath.pop();
    			var subjoinOnFieldBase = subjoinOnFieldPath.join('.');
    			var subjoinOnAliasItem = _.find(subjoinContext.hashAlias, function(obj) { return obj.key === subjoinOnFieldBase; });
    			var subjoinOnFieldKey = subjoinOnAliasItem.alias + '$' + subjoinOnFieldName;

    			// Add calculated column
    			if(objectPath.get(fieldValue, '$count')) {
    				subqueryBuilder.addColumn(knex.raw('COUNT(`' + joinAlias + '`.`' + subjoinOnFieldKey + '`) AS `' + calculatedColumn + '`'));
    			} else if(objectPath.get(fieldValue, '$sum')) {
    				subqueryBuilder.addColumn(knex.raw('SUM(`' + joinAlias + '`.`' + subjoinOnFieldKey + '`) AS `' + calculatedColumn + '`'));
    			} else if(objectPath.get(fieldValue, '$avg')) {
    				subqueryBuilder.addColumn(knex.raw('AVG(`' + joinAlias + '`.`' + subjoinOnFieldKey + '`) AS `' + calculatedColumn + '`'));
    			}

    			// Add join
				subqueryBuilder.addJoin('raw', [
					'LEFT JOIN (' + subjoinBuilder.toString() + ') AS `' + joinAlias + '`' +
					'ON `' + self.queryBuilder.getAlias() + '`.`' + parentAlias + '$' + model.getPrimaryKey() + '` = `' + joinAlias + '`.`' + subjoinOnFieldKey + '`'
				]);
    	...
    	// If field is one2many relationship
    		...
    		// Add required field
    		self.queryBuilderColumns[parentAlias + '.' + model.getPrimaryKey()] = parentAlias + '$' + model.getPrimaryKey();
    		self.requestedFields.push(parentAlias + '$' + model.getPrimaryKey());
...
var _parseFilterKey = function(model, itemKey, parentAlias, parentField, isAnAlias) {
	...
	// Set data
	var self = this;
	var absoluteHashAliasItem = null;
	var itemKeyBefore = '';
	var itemKeyAfter = '';
	...
	// If absolute hash alias item is undefined
...
var _parseFilterValue = function(model, itemValue, parentAlias, parentField) {
	...
	// If LIKE
		...
		whereClause += ' LIKE \'%' + itemValue['$like'] + '%\' ';
	}

	// If NOT LIKE
	else if(_.contains(_.keys(itemValue), '$nlike')) {
		whereClause += ' NOT LIKE \'%' + itemValue['$nlike'] + '%\' ';
	}
	...
	// If CURRENT WEEK / MONTH / YEAR
		...
		whereClause += ' >= CAST(\'' + moment().startOf('isoweek').format('YYYY-MM-DD') + '\' AS DATE) ';
	} else if(_.contains(_.keys(itemValue), '$currentmonth')) {
		whereClause += ' >= CAST(\'' + moment().startOf('month').format('YYYY-MM-DD') + '\' AS DATE) ';
	} else if(_.contains(_.keys(itemValue), '$currentyear')) {
		whereClause += ' >= CAST(\'' + moment().startOf('year').format('YYYY-MM-DD') + '\' AS DATE) ';
	}

	// If LAST WEEK / MONTH / YEAR
	else if(_.contains(_.keys(itemValue), '$lastweek')) {
		whereClause += ' BETWEEN CAST(\'' + moment().startOf('isoweek').subtract(1, 'week').format('YYYY-MM-DD') + '\' AS DATE) AND CAST(\'' + moment().endOf('isoweek').subtract(1, 'week').format('YYYY-MM-DD') + '\' AS DATE) ';
	} else if(_.contains(_.keys(itemValue), '$lastmonth')) {
		whereClause += ' BETWEEN CAST(\'' + moment().startOf('month').subtract(1, 'month').format('YYYY-MM-DD') + '\' AS DATE) AND CAST(\'' + moment().endOf('month').subtract(1, 'month').format('YYYY-MM-DD') + '\' AS DATE) ';
	} else if(_.contains(_.keys(itemValue), '$lastyear')) {
		whereClause += ' BETWEEN CAST(\'' + moment().startOf('year').subtract(1, 'year').format('YYYY-MM-DD') + '\' AS DATE) AND CAST(\'' + moment().endOf('year').subtract(1, 'year').format('YYYY-MM-DD') + '\' AS DATE) ';
	}
