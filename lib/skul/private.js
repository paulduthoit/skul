...
var _parseFields = function(model, fields, parentAlias, parentField) {
    ...
    _.each(fields, function(fieldValue, fieldKey) {
    	...
    	// If field is one2one custom relationship
    		...
    		// Set data
    		var mainBuilderAlias = self.queryBuilder.getAlias();
    		var fieldRelationship = model.getRelationship(fieldKey);
    		var relForeignKey = fieldRelationship.foreignKey;
		    ...
		    if(fieldRelationship.key) {
		    	self.queryBuilderColumns[parentAlias + '.' + fieldRelationship.key] = parentAlias + '$' + fieldRelationship.key;
		    } else {
			...
			subqueryBuildingTasks.push(function() {
				subqueryBuilder.addJoin('left', [
					fieldRelationship.model['$model'].getTableName() + ' AS ' + fieldRelationship.model['$alias'],
					self.queryBuilder.getAlias() + '.' + parentAlias + '$' + (fieldRelationship.key || model.getPrimaryKey()),
					relForeignKey['$alias'] + '.' + relForeignKey['$key']
				]);
			});
    		...
    		subqueryBuildingTasks.push(function() {
    			...
    			// Set data
    			var context = {
					mainModel: fieldRelationship.model['$model'],
			    ...
			    // Push alias
			    self.hashAlias.push({ alias: customAlias, key: fieldKey, model: context.mainModel });

				// Check if value request more
				if(objectPath.get(fieldValue, '$fields')) {
					_parseFields.call(context, fieldRelationship.model['$model'], fieldValue['$fields'], '$QB', '');
				} else {
					_parseFields.call(context, fieldRelationship.model['$model'], { '$all': 1 }, '$QB', '');
				}

				// Set columns
				subqueryBuilder.setColumns(_.map(context.queryBuilderColumns, function(value, key) { return key.replace('$QB', fieldRelationship.model['$alias']) + ' AS ' + value.replace('$QB', customAlias); }));

				// Add requested fields
				_.each(context.requestedFields, function(value) {
					self.requestedFields.push(value.replace('$QB', customAlias));
				});
			...
			// If join is provided
			if(fieldRelationship.join instanceof Array) {
			
				// Loop over join
				_.each(fieldRelationship.join, function(joinItem) {
			...
			else if(fieldRelationship.joinRaw instanceof Array) {
...
var _parseFilterValue = function(model, itemValue, parentAlias, parentField) {
	...
	// If <>
		...
		if(parsedValue === null) {
			...
			whereClause += ' != ' + knex.raw('?', [ parsedValue ]) + ' ';
		}
	...
	// If LIKE
		...
		whereClause += ' LIKE ' + knex.raw('?', [ '%' + itemValue['$like'] + '%' ]) + ' ';
	}

	// If NOT LIKE
	else if(_.contains(_.keys(itemValue), '$nlike')) {
		whereClause += ' NOT LIKE ' + knex.raw('?', [ '%' + itemValue['$nlike'] + '%' ]) + ' ';
	}
...
var _parseValue = function(value) {
	...
	if(isNaN(value['$value'])) {
		parsedValue += knex.raw('?', [ String(value['$value']) ]);
	} else {
		parsedValue += knex.raw('?', [ Number(value['$value']) ]);
	}
