...
var _parseFilterItem = function(queryBuilder, item, hashAlias, alias, alreadyFullfill) {
	...
	// Else
		...
		// Set real field base
		fieldBase = _.reject([ hashAlias[alias], fieldBase ], function(i) { return !i; }).join('.');
...
var _addRequiredJoins = function(queryBuilder, parentKeyPath, hashAlias, alias) {
	...
	if(!_.contains(_.values(hashAlias), fieldRoot)) {
	    ...
	    if(_.contains([ 'belongsTo', 'hasMany', 'hasOne' ], relType)) {
		    ...
		    // Push alias
		    hashAlias[relAlias] = _.reject([ hashAlias[alias], fieldRoot ], function(obj) { return !obj; }).join('.');
	    ...
	    var relationship = self.getRelationship(fieldRoot);
	    var relModel = relationship.model;
	    var relAlias = _.invert(hashAlias)[fieldRoot];
...
// Add custom join
var _addCustomJoin = function(queryBuilder, model, join, on, hashAlias, alias) {

	// Set hashAlias
	if(join.table instanceof Array) {
		_.each(join.table, function(joinTableItem) {
			if(!_.contains(_.pluck(hashAlias, 'refAlias'), joinTableItem['$alias'])) {
				hashAlias.push({ alias: (_.max(_.pluck(hashAlias, 'alias').concat(0)) + 1), refAlias: joinTableItem['$alias'], model: joinTableItem['$model'] });
			}
		});
	} else {
		hashAlias.push({ alias: 1, refAlias: relJoinTable['$alias'], model: relJoinTable['$model'] });
	}
	...
	if(join.table instanceof Array) {
		joinRaw += '(';
		joinRaw += _.map(join.table, function(obj) { return '`' + obj['$model'].getTableName() + '` AS `' + _.findWhere(hashAlias, { refAlias: obj['$alias'] }).alias + '`' }).join(', ');
		joinRaw += ')';
	}

	// If join table is not an array
	else {
		joinRaw += '`' + join.table['$model'].getTableName() + '` AS `' + _.findWhere(hashAlias, { refAlias: join.table['$alias'] }).alias + '`';
	}
	...
	_.each(on, function(onItem) {
		...
		// If item is object
			...
			joinRaw += '`' + _.findWhere(hashAlias, { refAlias: onItem['$alias'] }).alias + '`.`' + onItem['$key'] + '`';
		}
