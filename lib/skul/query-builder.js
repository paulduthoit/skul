...
QueryBuilder.prototype.parseFromRaw = function(queryBuilder) {
	...
	if(this.fromRaw !== null) {
		...
		if(typeof this.fromRaw === 'object' && this.fromRaw.queryBuilder instanceof QueryBuilder) {
			queryBuilder = queryBuilder.from(knex.raw('(' + this.fromRaw.queryBuilder.toString() + ') AS `' + this.fromRaw.alias + '`'));
		} else {
...
QueryBuilder.prototype.addGroupBy = function(value) {
	...
	// Check if already exist
	var groupByItemRef = _.findIndex(this.groupBy, function(obj) {
		return obj === value;
	});
...
QueryBuilder.prototype.setGroupBy = function(value) {
	this.groupBy = value;
};
