...
module.exports.parseFields = function(fields) {
	...
	_.each(fields, function(obj, key) {
		...
		// If $all
		if(key === '$all' && obj === 1) {
			parsedFields = parsedFields.concat(structureColumns);
		}
		...
		// If $all
			...
			// Push simple keys
			parsedFields = parsedFields.concat(_.intersection(structureColumns, defaultKeys));

			// Push virtual default keys
			_.each(_.intersection(virtualKeys, defaultKeys), function(key) {
				parsedFields = parsedFields.concat(_.keys(virtuals[key].requiredKeys));
			});

			// Push belongsTo relationship default keys
			_.each(_.intersection(belongsToRelationshipKeys, defaultKeys), function(key) {
		...
		else if(obj !== 0 && !_.contains(allRelationshipKeys, key) && !_.contains(customKeys, key) && _.contains(structureColumns, key)) {
...
module.exports.parseVirtuals = function(fields) {
	...
	_.each(fields, function(obj, key) {
		...
		// If key is $all
		else if(key === '$all' && _.intersection(structureColumns, virtualKeys).length > 0) {

			// Loop over each intersection between availables and virtuals
			_.each(_.intersection(structureColumns, virtualKeys), function(fieldKey) {
				parsedFields[fieldKey] = obj;
			});
...
module.exports.parseCustoms = function(fields) {
	...
	_.each(fields, function(fieldObj, fieldKey) {
		...
		// If $all
			...
			_.each(defaultCustomKeys, function(key) {
				parsedFields[key] = customs[key];
			});
		}
...
module.exports.parseRelationships = function(type, fields) {
	...
	// Check arguments
	if(!(typeof type === "string" && _.contains([ 'belongsTo', 'hasOne', 'hasOneCustom', 'hasMany', 'hasManyCustom' ], type)) && typeof type !== "undefined")
		throw new Error("type have to be a string defined as belongsTo, hasOne, hasOneCustom, hasMany or hasManyCustom");
	if(typeof fields !== "object")
...
module.exports.parseSelectOptions = function(options) {
    ...
    if(options.orderby !== null) {
    	...
    	// Data
    	var _getStructureColumnsLvl = 0;
    	var maxStructureColumnsLvl = _.max(_.map(options.orderby, function(value) { return value.split('.').length; }));

    	// Get available fields
    	var _getStructureColumns = function(model) {

    		// Increment field lvl
    		_getStructureColumnsLvl++;

    		// Data
    		var structureColumns = [];

    		// Get main available fields
    		var mainStructureColumns = model.getStructureColumns();
    		var mainCustomKeys = model.getCustomKeys();

    		// Add main and custom fields
    		structureColumns = structureColumns.concat(mainStructureColumns, mainCustomKeys);

    		// Check lvl
    		if(_getStructureColumnsLvl > maxStructureColumnsLvl) {
    			_getStructureColumnsLvl--;
    			return;
    		}
    		...
    		_.each(belongsToRelationships, function(rel, key) {
    			...
    			var relStructureColumns = _getStructureColumns(rel.model);
    				relStructureColumns = _.map(relStructureColumns, function(obj) { return key + '.' + obj; });

    			// Add relationship available fields
    			structureColumns = structureColumns.concat(relStructureColumns);
    		...
    		// Add relationship hasOneCustom fields
    		var hasOneCustomRelationships = model.getRelationships('hasOneCustom');

    		// Loop over relationships
    		_.each(hasOneCustomRelationships, function(rel, key) {
    	...
    	// Get available fields
    	var structureColumns = _getStructureColumns(self);

        // Check orderby
        var orderByWrongKeys = _.difference(_.map(options.orderby, function(value) { return value.replace(/^\-/, ''); }), structureColumns);
