...
module.exports.searchToFilter = function(filter, searchString, fields) {
	...
	_.each(searchFields, function(fieldItem) {
		...
		// Add to where clause
		if(splittedSearchString.length === 1) {
			whereClause = {};
			whereClause[fieldItem] = { '$like': dbConnection.escape(searchString) };
		} else {
			whereClause = { '$and': [] };
			_.each(splittedSearchString, function(item) {
				whereItem = {};
				whereItem[fieldItem] = { '$like': dbConnection.escape(item) };
				whereClause['$and'].push(whereItem);
			});
		}
